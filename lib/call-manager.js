...
class CallManager extends Emitter {
  constructor(opts) {
    ...
    // set to true when we get our first answer
    this.oneCallConnected = false;
  ...
  attemptOne(target) {
    ...
    const p = this.srf.createUAC(uri,
      ...
      .then((dlg) => {
        if (this.oneCallConnected) {
          // race condition: someone else already answered before we could cancel this one
          this.logger.info(`CallManager#attemptOne: terminating call to ${uri} (sorry second place finisher!)`);
          dlg.destroy();
          return null;
        }
        this.oneCallConnected = true;
        this.cip.delete(uri);
      ...
      .catch((err) => {
        if (!this.callerGone && 487 !== err.status) {
