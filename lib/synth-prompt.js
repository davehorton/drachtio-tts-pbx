  ...
  return function(text, voice) {
    ...
    return fs.pathExists(filePath)
      .then((exists) => {
        ...
        Object.assign(speechOpts, {input: text.startsWith('<speak>') ? {ssml: text} : {text}});
        logger.debug(`synthesizing: ${JSON.stringify(speechOpts)}`);
        return client.synthesizeSpeech(speechOpts);
      })
      .then((responses) => {
